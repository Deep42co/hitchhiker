...
namespace Deep42\Hitchhiker\Steps;
...
class ConfigureEnvironment implements InstallerStep
{
    ...
    public function validate(array $data = []): bool
    {
        ...
        if ($connection !== 'sqlite' && empty($data['host'])) {
            throw new RuntimeException(__('installer::installer.environment_host_required'));
        }

        if (empty($data['database'])) {
            throw new RuntimeException(__('installer::installer.environment_database_required'));
        }
